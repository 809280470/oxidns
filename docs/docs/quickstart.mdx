...
Linux x86_64 / ARM64 musl 额外提供 `minimal` 和 `standard` slim 压缩包。`minimal` 只包含 `oxidns`、`config.yaml` 和 `LICENSE`；`standard` 还包含 WebUI 静态文件，并内置 query_recorder 与 upgrade 能力。各预设编译版的完整能力差异见[自定义编译](./custom-build#预设能力矩阵)。
