...
|---|---|---|
| [模版仓库 + GitHub Actions](#template-build) | "我想每次上游发布就自动重编自己版本",但不想维护 fork | ❌ |
| [本地 `cargo build`](#local-build) | 手头有 Rust 环境,只需要一次性产出一个定制二进制 | ✅ |
| [fork 整个项目](#fork-build) | 需要持续修改源码 / 长期偏离上游 | ✅ |
...
## Bundles 预设组合 \{#preset-capability-matrix}
...
二进制大小会随 Rust 工具链、依赖版本、目标平台、链接方式和 UPX
处理变化。需要容量规划时,应测量目标 release asset 或相同构建参数下的
本地产物,不要把历史版本的大小当成固定承诺。
...
- `network.outbound` resolver:DoT、DoH (HTTP/2)、DoQ nameserver
- Provider:`geoip`、`geosite`、`v2ray_dat`、`adguard_rule`、
  `dynamic_domain_set`
- Executor:`arbitrary`、`response`、`cron`、`download`、`http_request`、
  `reverse_lookup`、`query_recorder`、`script`、`ip_selector`、
  `learn_domain`
...
- 入站 / 上游 / `network.outbound` resolver:DoH HTTP/3
- Executor:`ros_address_list` / `ros_route` (MikroTik)、`ipset`、`nftset`
...
| `resolver-dot` | 允许 `network.outbound.resolver.nameservers` 使用 DoT |
| `resolver-doh` | 允许 `network.outbound.resolver.nameservers` 使用 DoH (HTTP/2) |
| `resolver-doq` | 允许 `network.outbound.resolver.nameservers` 使用 DoQ |
| `resolver-doh3` | 允许 `network.outbound.resolver.nameservers` 使用 DoH HTTP/3(需 `resolver-doh`) |
...
| Feature | 插件 | 主要依赖 |
|---|---|---|
| `plugin-mikrotik` | `ros_address_list` + `ros_route` 聚合 feature | `mikrotik-rs` |
...
## 方式二:本地 `cargo build` \{#local-build}
...
# 只在 minimal 上同时加入两个 MikroTik 插件
