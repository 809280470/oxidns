...
`enable` 和 `disable` 均为幂等操作，不需要请求体。matcher 被关闭后不会执行内部匹配逻辑，其基础结果固定为 `false`；现有取反语义仍会继续应用，因此 `!$match_cn` 在该 matcher 关闭时为 `true`。

开关只属于当前 runtime，不写入 YAML；应用级 reload 或进程重启后恢复为开启。`sequence` 和 `any_match` 内的 quick-setup matcher 使用内部 `qs.match...` tag，不注册控制接口。如需运行时控制，应先将其提取为 `plugins:` 中的独立 matcher 并通过 `$tag` 引用。

WebUI 将关闭操作呈现为“临时旁路”，并在执行前明确提示正向引用与取反引用的不同影响；恢复匹配不需要修改或重新应用 YAML。
