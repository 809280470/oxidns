...
- 同一 OxiDNS 进程内，重载会把同一 ownership namespace 的写权限移交给新实例。不同进程不能同时使用相同的插件 tag、注释前缀和目标列表；滚动发布时应使用不同 ownership namespace，或确保旧实例停止写入。
...
- 同一 OxiDNS 进程内，重载会先停止并排空旧 manager，再把同一 ownership namespace 的写权限和待处理观测移交给新实例。不同进程不能同时使用相同插件 tag、注释前缀和 routing table；滚动发布时应使用不同 ownership namespace，或确保旧实例停止写入。
