...
- 模式说明：
  ...
  - `balanced`：默认模式。包含请求类型且位于原始 QNAME/CNAME 链尾的完整答案立即胜出；`NXDOMAIN` 或 NODATA 会短暂等待是否有正向答案。
  - `prefer_positive`：正向答案立即胜出；负向答案等待已发起的并发上游全部结束后再返回。
  - `consensus`：正向答案立即胜出；负向答案需要两个上游确认，若并发数不足则退化为 `prefer_positive`。
...
- `forward` 不执行隐藏式 fallback 或 CNAME chase；需要更多上游竞争时请显式提高 `concurrent`，或在策略中配置 `fallback`。
