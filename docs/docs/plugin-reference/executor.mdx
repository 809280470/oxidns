...
- tag: ros_address_list_main
  ...
  args:
    ...
    # API 密码
    password: "secret"
    # 省略 tls 时使用明文 API（通常 8728）；启用后使用 API-SSL（通常 8729）
    tls:
...
- 作用：启用 RouterOS API-SSL。`server_name` 可覆盖证书名称，`ca` 加载自签名/私有 CA；只有无法建立可信证书链时才使用 `insecure: true`，且 `insecure` 不能与 `ca` 同时配置。
...
- 类型：`usize`；必填：否；默认值：`65536`
- 作用：限制本地已知动态地址键数量，persistent 地址不计入。RouterOS 已有动态项超过上限时会保留，但降到上限以下前拒绝新键。
- 注意事项：`fixed_ttl: 0` 不会自然过期，必须结合合理上限和对账规划容量。
...
将成功 DNS 应答中的 A/AAAA 地址写入 RouterOS 指定策略路由表。插件是 continuation executor，应放在产生最终应答的 executor 之前。
...
- `min_ttl`/`max_ttl` 默认分别为 `60`/`3600` 秒。`fixed_ttl` 覆盖 DNS TTL；其中 `fixed_ttl: 0` 表示动态路由不按时间过期，但同一域名后续应答移除该 IP 时会撤销其引用。
- `ros_route` 与 `ros_address_list` 都会收集 Answer 区中所有已启用地址族的 A/AAAA 记录，不再重建 CNAME 链或用 CNAME TTL 截断地址 TTL。对 `ros_route` 而言，A 查询只替换/撤销 IPv4 路由，AAAA 查询只替换/撤销 IPv6 路由；但同一 Answer 中的两种地址都会作为新增或刷新候选。
- `persistent_route` 是独立于 DNS 的常驻 IP/CIDR 集合；文件内容每分钟重载并周期对账。
- RouterOS 不可达时 DNS 服务仍会启动，路由操作在后台重试。`async: false` 只等待一次同步尝试，失败不会改变 DNS 应答。
- `conntrack_guard` 默认关闭。启用后，动态撤销、过期和移除 persistent 配置前会检查 RouterOS conntrack；目标地址仍有任意跟踪连接时保留路由并在 30 秒后重试。CIDR 会检查网段内连接。连接跟踪关闭、RAW `notrack` 流量或查询失败无法证明安全时都会保留路由。关闭清理、校验残留、重复项和禁用地址族清理仍会立即删除。
- `max_entries` 默认 `65536`，限制动态路由和域名绑定状态；persistent 路由不计入。`fixed_ttl: 0` 路由不会自然过期，应为其预留容量。
- 动态路由以 RouterOS 实际状态为准：用户删除后由下一次对账接受删除，下一次 DNS 观测再创建；不会在每次 DNS 请求中读取路由表。persistent 路由仍以配置为准。
- 重启恢复 shared-IP 时，RouterOS comment 只保存一个代表域名。有限 TTL 路由会保守保留到 comment 最大有效期；timeless 路由按“仍可能存在未知 owner”处理，只能由远端删除、shutdown cleanup 或所有权配置变化清理。
- `tls` 启用 API-SSL（通常 `8729`）；自签名证书优先配置 `tls.ca`，仅在确实无法验证时使用 `insecure: true`。省略 `tls` 时使用明文 API（通常 `8728`）。
- `cleanup_on_shutdown: true` 会删除该插件 tag 所拥有的动态和常驻路由；设为 `false` 时保留并在下次启动恢复。
