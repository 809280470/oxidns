...
把应答 IP 同步到 RouterOS address-list，支持动态项、常驻项、启动时文件加载和关闭时清理。这是推荐的 RouterOS 联动方式，适合让 firewall、mangle 或 routing rule 消费统一地址集合。
...
- 类型：`bool`；必填：否；默认值：`true`
- 作用：控制插件退出时是否清理由其管理的条目。启用后，插件在正常关闭阶段会删除自身写入并可识别归属的 RouterOS 地址项，全部清理共用 30 秒预算；超时后进程继续关闭并报告错误。
- 影响：关闭该选项后，已写入条目会继续保留在 RouterOS 中，适合要求策略状态跨进程重启保留的场景。生产重启或滚动发布需要策略连续性时建议设为 `false`。
...
- 动态项不参与启动或周期全表对账。用户手工删除、修改动态项后插件不主动纠偏；后续只有正常 TTL 刷新或新写入才可能覆盖该变化。
- 有限 TTL 动态项在有效 TTL 的 75% 处允许再次刷新；`fixed_ttl: 0` 的永久项不做周期刷新。
...
- 应用级 reload 对该插件采用 shutdown/restart 语义：候选 runtime 可提前完成无副作用的初始化，但旧实例会先完整 shutdown，新实例随后才激活；旧实例待处理观测不移交。`cleanup_on_shutdown: true` 时会清理旧实例拥有的条目，重建期间可能出现策略空窗；如需在 reload 期间保留 RouterOS 条目，请设为 `false`。不同进程不能同时使用相同的插件 tag、注释前缀和目标列表；滚动发布时应使用不同 ownership namespace，或确保旧实例停止写入。

</Admonition>

## `ros_route`

将成功 DNS 应答中的 A/AAAA 地址写入 RouterOS 指定策略路由表。这是需要逐 IP 静态路由时使用的高级方式；一般策略联动优先使用 `ros_address_list`。插件是 continuation executor，应放在产生最终应答的 executor 之前。
...
- 每 180 秒的定时对账只处理 `persistent`；没有持久项时不发起周期路由表查询。动态路由不会因用户手工修改而触发周期纠偏。
...
- 应用级 reload 对该插件采用 shutdown/restart 语义：候选 runtime 可提前完成无副作用的初始化，但旧实例会先完整 shutdown，新实例随后才激活；旧实例待处理观测不移交。`cleanup_on_shutdown: true` 时会清理旧实例拥有的路由，重建期间可能出现策略空窗；如需在 reload 期间保留 RouterOS 路由，请设为 `false`。不同进程不能同时使用相同插件 tag、注释前缀和 routing table；滚动发布时应使用不同 ownership namespace，或确保旧实例停止写入。
...
- 动态租约在剩余 TTL 的 75% 或上次成功写入后 5 分钟（取较早者）写回 comment，以兼顾管理面写入量和异常重启恢复精度。
