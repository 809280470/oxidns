...
- manager 负责：
  ...
  - 持久项一致性维护
  - 关闭清理
- 动态项以 RouterOS 实际状态为准：用户删除后，周期对账清除本地写入抑制状态，下一次 DNS 观测才重新写入；插件不会为每次 DNS 请求读取 RouterOS。persistent 项则以配置为准，被删除后会由对账恢复。
...
- RouterOS 不可达时 DNS 服务仍会启动，路由操作在后台重试。`async: false` 只等待一次同步尝试，失败不会改变 DNS 应答。
...
- 动态路由以 RouterOS 实际状态为准：用户删除后由下一次对账接受删除，下一次 DNS 观测再创建；不会在每次 DNS 请求中读取路由表。persistent 路由仍以配置为准。
...
- 删除前会重新读取路由并比较完整快照（comment、gateway、distance、disabled 等）。但 RouterOS 最终只提供按内部 ID 删除，不支持原子 compare-and-delete；因此不要在其它进程或 RouterOS 管理界面中并发修改插件拥有的路由。用户直接删除动态路由仍是支持的操作，对账会接受删除事实。
