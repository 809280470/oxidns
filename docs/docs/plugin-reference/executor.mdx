...
- 应用级 reload 按 shutdown 处理：先停止并销毁旧 runtime，再创建新 runtime，不移交待处理观测。`cleanup_on_shutdown: true` 时会先清理旧实例拥有的条目，重建期间可能出现策略空窗；如需在 reload 期间保留 RouterOS 条目，请设为 `false`。不同进程不能同时使用相同的插件 tag、注释前缀和目标列表；滚动发布时应使用不同 ownership namespace，或确保旧实例停止写入。
...
- 应用级 reload 按 shutdown 处理：先停止并销毁旧 runtime，再创建新 runtime，不移交待处理观测。`cleanup_on_shutdown: true` 时会先清理旧实例拥有的路由，重建期间可能出现策略空窗；如需在 reload 期间保留 RouterOS 路由，请设为 `false`。不同进程不能同时使用相同插件 tag、注释前缀和 routing table；滚动发布时应使用不同 ownership namespace，或确保旧实例停止写入。
