...
| 插件 | 作用 |
| --- | --- |
| [`sequence`](executor.mdx#sequence) | 把多个 matcher 和 executor 编排成一条流水线，是最常用的入口执行器。 |
| [`fallback`](executor.mdx#fallback) | 在主路径失败或过慢时，切换到备用路径。 |

### 请求处理

| 插件 | 作用 |
| --- | --- |
| [`forward`](executor.mdx#forward) | 向上游发起 DNS 查询。 |
...
| [`cron`](executor.mdx#cron) | 后台调度一组 executor，按 cron 表达式或固定间隔触发任务。 |
...
| 插件 | 作用 |
| --- | --- |
| [`qname`](matcher.mdx#qname) | 匹配请求中的查询域名。 |
| [`question`](matcher.mdx#question) | 按 provider 的 `contains_question` 语义匹配请求中的 question。 |
| [`qtype`](matcher.mdx#qtype) | 匹配请求中的 qtype。 |
| [`qclass`](matcher.mdx#qclass) | 匹配请求中的 qclass。 |
| [`client_ip`](matcher.mdx#client_ip) | 匹配客户端来源 IP。 |
| [`ptr_ip`](matcher.mdx#ptr_ip) | 从 PTR 请求名中解析 IP 并做匹配。 |

### 响应维度

| 插件 | 作用 |
| --- | --- |
| [`resp_ip`](matcher.mdx#resp_ip) | 匹配响应 answers 中的 A / AAAA IP。 |
| [`cname`](matcher.mdx#cname) | 匹配响应中的 CNAME 目标域名。 |
| [`rcode`](matcher.mdx#rcode) | 匹配当前响应的 rcode。 |
| [`has_resp`](matcher.mdx#has_resp) | 只要上下文中已有响应就命中。 |
| [`has_wanted_ans`](matcher.mdx#has_wanted_ans) | 判断响应 answers 中是否包含与请求 qtype 对应的记录。 |
...
| 插件 | 作用 |
| --- | --- |
| [`any_match`](matcher.mdx#any_match) | 组合多个 matcher 表达式，任意一个命中即返回 `true`。 |
| [`_true`](matcher.mdx#_true) | 恒为真。 |
| [`_false`](matcher.mdx#_false) | 恒为假。 |
...
| [`geoip`](provider.mdx#geoip) | 从 v2ray-rules-dat 的 `geoip.dat` 中提取一个或多个 code，并编译成可复用 IP / CIDR 集合。 |
