...
|---|---|---|
| [Template repo + GitHub Actions](#template-build) | "Rebuild my own version every time upstream releases," without forking | ❌ |
| [Local `cargo build`](#local-build) | One-off custom binary, you already have Rust installed | ✅ |
| [Fork the whole project](#fork-build) | Long-term source modifications, diverging from upstream | ✅ |
...
## Bundles \{#preset-capability-matrix}
...
Binary size varies with the Rust toolchain, dependency versions, target,
linking mode, and UPX processing. For capacity planning, measure the target
release asset or a local artifact built with the same flags instead of treating
historical sizes as a fixed commitment.
...
- Executors: `sequence`, `forward`, `cache`, `fallback`, `hosts`, `redirect`,
  `prefer_ipv4`, `prefer_ipv6`, `ecs_handler`, `ttl`, `drop_resp`, `black_hole`,
  `debug_print`, `reload`
...
- `network.outbound` resolver: DoT, DoH (HTTP/2), and DoQ nameservers
- Providers: `geoip`, `geosite`, `v2ray_dat`, `adguard_rule`,
  `dynamic_domain_set`
- Executors: `arbitrary`, `response`, `cron`, `download`, `http_request`,
  `reverse_lookup`, `query_recorder`, `script`, `ip_selector`, and
  `learn_domain`
...
- Inbound / upstream / `network.outbound` resolver: DoH HTTP/3
- Executors: `ros_address_list` / `ros_route` (MikroTik), `ipset`, `nftset`
...
| `resolver-dot` | Allow `network.outbound.resolver.nameservers` to use DoT |
| `resolver-doh` | Allow `network.outbound.resolver.nameservers` to use DoH (HTTP/2) |
| `resolver-doq` | Allow `network.outbound.resolver.nameservers` to use DoQ |
| `resolver-doh3` | Allow `network.outbound.resolver.nameservers` to use DoH HTTP/3 (requires `resolver-doh`) |
...
|---|---|---|
| `plugin-mikrotik` | Aggregate feature for `ros_address_list` + `ros_route` | `mikrotik-rs` |
| `plugin-ros-address-list` | `ros_address_list` | `mikrotik-rs` |
| `plugin-ros-route` | `ros_route` | `mikrotik-rs` |
...
| `plugin-dynamic-domain` | `dynamic_domain_set` provider + `learn_domain` executor | — |
| `plugin-ip-selector` | `ip_selector` | — |
| `plugin-script` | `script` | — |
| `plugin-arbitrary` | `arbitrary` | `oxidns-zoneparser` |
| `plugin-response` | `response` | `oxidns-zoneparser` |
...
## Path 1: template repo (automated builds) \{#template-build}
...
Identical to upstream's `release.yml` — 13 targets:
...
## Path 2: local `cargo build` \{#local-build}
...
# minimal + both MikroTik plugins on top
cargo build --release --no-default-features --features "minimal,plugin-mikrotik"

# Or add only one of the plugins
cargo build --release --no-default-features --features "minimal,plugin-ros-address-list"
...
## Path 3: fork the project \{#fork-build}
...
automatically. Pin to an actual tag in the form `@vX.Y.Z` for a fixed version.
