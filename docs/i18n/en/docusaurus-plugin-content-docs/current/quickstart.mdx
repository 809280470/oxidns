...
After the installer completes, run a minimal verification before reading the advanced installation options below:
...
If the health API works but DNS queries fail, go directly to [Operations and Troubleshooting](operations.md). Before allowing other devices to reach the WebUI or API, read [Security Hardening](security.md).

On OpenWrt, the installer only installs the LuCI app. Install and start the OxiDNS core from `Services -> OxiDNS -> Core`, then verify it from the LuCI status page and with a DNS query.
...
```bash
curl -fsSL https://oxidns.org/install.sh | sudo env OXIDNS_VERSION=vX.Y.Z sh
```
...
```powershell
$env:OXIDNS_VERSION = "vX.Y.Z"; irm https://oxidns.org/install.ps1 | iex
```
...
- `oxidns-armv7-unknown-linux-musleabihf.tar.gz`
...
| Linux ARMv7 / armv7l | `oxidns-armv7-unknown-linux-musleabihf.tar.gz` | Fits ARMv7 devices |
| Generic 32-bit ARM Linux | `oxidns-arm-unknown-linux-musleabihf.tar.gz` | Fits older ARM boards |
...
- `Linux` + `armv7l`: use `armv7-unknown-linux-musleabihf`
...
Before starting, install Docker Engine and the Docker Compose plugin (which provides `docker compose`), then verify that both commands below work. See the [official Docker installation guide](https://docs.docker.com/engine/install/):
...
Create an empty deployment directory, then export the default configuration that matches the current image version:
...
Use the export command only to create the configuration for a first-time deployment. Keep and reuse the existing `config.yaml` when upgrading or restarting a deployment; do not overwrite it with the image default.

Create `config.yaml` as a regular file before starting the container. If the bind-mount source does not exist, Docker may create a directory with that name and then fail because it cannot mount the directory at `/etc/oxidns/config.yaml`.

<Admonition type="note" title="DNS ports in the configuration">

The image configuration listens on container port `53`. The development `config.yaml` at the repository root listens on `5335` by default, so do not use it unchanged with the `53:53` mappings below. To use the repository configuration, either change the UDP and TCP server listeners to `:53` or adjust the container-side port mappings accordingly.

</Admonition>

<Admonition type="warning" title="Protect the management API">

The examples below publish the management API and WebUI on host port `9199`, while the exported default configuration does not enable authentication. Before allowing access from other devices, enable `api.http.auth` in `config.yaml` and restrict sources with a firewall. For local-only access, change the mapping to `127.0.0.1:9199:9199/tcp`.

</Admonition>

### Start With docker run

Confirm that the current directory contains the generated `config.yaml`, then run:
...
<Admonition type="note" title="Port mapping">

If host port 53 is already used by systemd-resolved, dnsmasq, or another DNS service, change the host side to `5353:53`, for example `-p 5353:53/udp -p 5353:53/tcp`. Keep the container side on port 53.

</Admonition>

The examples use the rolling `latest` tag. For production deployments, pin an explicit release tag to avoid an unexpected upgrade when the image is pulled again.
...
Check the container, API, WebUI, and DNS:
...
Open the WebUI at `http://SERVER_IP:9199/`. If Basic Auth is enabled, enter the matching username and password on the WebUI settings page.

### Docker Compose Example

For Compose-based port mapping and config management, first follow "Prepare the Configuration File" so that `config.yaml` exists in the current directory, then create this `docker-compose.yml`:
...
Run the health check in another terminal:
