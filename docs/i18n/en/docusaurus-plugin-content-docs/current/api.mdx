...
* Reload, restart, and shutdown control
* Runtime log reads and SSE streaming
* Optional release-upgrade control
...
POST /api/plugins/<matcher_tag>/enable
POST /api/plugins/<matcher_tag>/disable
```

All three endpoints return the same status shape:
...
`enable` and `disable` are idempotent and require no request body. A disabled matcher skips its internal predicate and produces a base result of `false`; existing negation still applies, so `!$match_cn` evaluates to `true` while that matcher is disabled.

The switch belongs only to the current runtime and is not written to YAML. An application reload or process restart restores the matcher to enabled. Quick-setup matchers inside `sequence` or `any_match` use internal `qs.match...` tags and do not register these endpoints. Extract a quick-setup expression into a standalone matcher under `plugins:` and reference it with `$tag` when runtime control is required.

The WebUI presents disabling as a temporary bypass and explains the different effects on positive and negated references before applying it. Restoring matching does not require editing or reapplying YAML.
