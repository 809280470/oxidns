...
Matchers declared in `plugins:` with stable tags can be enabled, disabled, and inspected through the management API or WebUI for the current runtime. A disabled matcher skips its internal predicate and produces a base result of `false`; outer negation still applies, so `!$matcher` evaluates to `true`.
