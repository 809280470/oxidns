...
Matchers declared in `plugins:` with stable tags can be enabled, disabled, and inspected through the management API for the current runtime. The WebUI presents disabling as a confirmed temporary bypass. A bypassed matcher skips its internal predicate and produces a base result of `false`; outer negation still applies, so `!$matcher` evaluates to `true`. Normal matching can be restored with one action.

This state is not written to the configuration, and a full reload or process restart restores the matcher to enabled. Quick-setup matchers do not have stable public tags and therefore do not expose runtime switches; extract the expression into a standalone matcher when control is required. See the [Management API](../api.mdx#matcher) for endpoint details.
